...
```sh
aws s3 mb s3://mycluster.in --region ap-south-1
```
...
```sh
export KOPS_CLUSTER_NAME=mycluster.ml
export KOPS_STATE_STORE=s3://myclucter.ml
```
...
kops create cluster --name mycluster.ml --zones ap-south-1a --dns private --node-count 2 --node-size t2.medium --master-count 1 --master-size t2.medium --cloud aws
